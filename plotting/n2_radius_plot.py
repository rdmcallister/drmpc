...
ax = axs
ax.semilogx(
    epsilons, np.average(J_i, axis=-1), label=r"$\tilde{\mathcal{J}}_{T}$", marker="."
)
J_min = np.average(J_i, axis=-1) - np.std(J_i, axis=-1)
J_max = np.average(J_i, axis=-1) + np.std(J_i, axis=-1)
ax.fill_between(epsilons, J_min, J_max, alpha=0.3)

J_sorted = np.sort(J_i, axis=-1)
J_min = J_sorted[:, 0]
J_max = J_sorted[:, -1]
ax.semilogx(epsilons, J_max, label=r"min/max", linestyle="dashed", color="C0")
ax.semilogx(epsilons, J_min, linestyle="dashed", color="C0")
